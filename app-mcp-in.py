...
class ScoringTool:
    ...
    def score_agent1(cls, flat: Dict[str, Any]) -> Dict[str, dict]:
        """Agent1: Heuristic-based scoring from alert metadata analysis"""
        scores = {}
    ...
    async def score_agent2_virustotal(self, flat: Dict[str, Any]) -> Dict[str, dict]:
        """Agent2: VirusTotal-based scoring using MCP integration"""
        scores = {}
        
        print("=== AGENT2 SCORING START (VirusTotal Analysis) ===")
        ...
        print(f"=== AGENT2 (VirusTotal) TOTAL SCORE: {agent2_total} ===")
        return scores
    ...
    def _process_checkpoint_data(self, cp_data: dict) -> dict:
        ...
        # 1. Risk score (direct numerical indicator)
        risk_score = cp_result.get("risk", 0)
        if isinstance(risk_score, (int, float)) and risk_score > 0:
            # Scale risk score: 0-100 -> 0-50 points
            scaled_score = min(risk_score * 0.5, 50)
        ...
        # 2. Reputation classification
        reputation = cp_result.get("reputation", {})
        classification = reputation.get("classification", "")
        confidence = reputation.get("confidence", "")
        severity = reputation.get("severity", "")
        
        if classification:
            if classification.lower() == "malware":
                base_score = 45
                ...
                scores["cp_malware_classification"] = {
                    "value": f"{classification} ({confidence})",
                    "risk_score": min(base_score, 60),
                    "description": f"Check Point classified as {classification} with {confidence} confidence"
                }
            elif classification.lower() in ["suspicious", "potentially_unwanted"]:
                base_score = 25
                if confidence.lower() == "high":
                    base_score += 8
                elif confidence.lower() == "medium":
                ...
                scores["cp_suspicious_classification"] = {
                    "value": f"{classification} ({confidence})",
                    "risk_score": min(base_score, 35),
                    "description": f"Check Point classified as {classification} with {confidence} confidence"
                }

        # 3. Severity level
        if severity and severity.lower() == "high":
            scores["cp_high_severity"] = {
                "value": severity,
                "risk_score": 20,
                "description": f"Check Point high severity rating"
            }

        # 4. Findings analysis
        findings = cp_result.get("findings", {})
        if findings:
            positives = findings.get("positives", 0)
            total = findings.get("total", 0)
            
            if positives > 0 and total > 0:
                detection_ratio = positives / total
                # Score based on detection ratio
                risk_score = min(detection_ratio * 30, 30)
                scores["cp_detection_ratio"] = {
                    "value": f"{positives}/{total}",
                    "risk_score": risk_score,
                    "description": f"Check Point detection ratio: {positives}/{total}"
                }
...
class FlexibleAlertInput:
    """Flexible alert input handler for various EDR formats"""
    
    def __init__(self, **data):
        self.raw_data = data
        self.alert_id = self._extract_alert_id()
        
    def _extract_alert_id(self):
        """Extract alert ID from various possible locations"""
        possible_keys = [
            "id", "alert_id", "threatId", "threat_id", 
            "incidentId", "incident_id", "alertId"
        ]
        
        for key in possible_keys:
            if key in self.raw_data:
                return str(self.raw_data[key])
        
        # Look in nested objects
        threat_info = self.raw_data.get("threatInfo", {})
        if threat_info and "threatId" in threat_info:
            return str(threat_info["threatId"])
            
        return "unknown_alert"
    
    def to_legacy_format(self):
        """Convert to normalized format expected by the scoring system"""
        # Return the raw data - the scoring system handles flattening
...
async def triage_alert(
    ...
    Analyzes alerts using 4 agents:
    - Agent1 (25%): Heuristic analysis from alert metadata
    - Agent2 (35%): VirusTotal threat intelligence  
    - Agent3 (20%): Check Point reputation analysis
    - Agent4 (20%): Cyberint intelligence analysis
    ...
    try:
        ...
        # Validate and normalize using flexible input handler
        try:
            ...
            if not any(key in alert_data for key in ['alert', 'threat', 'id', 'alert_id', 'threatInfo']):
                # Add a synthetic alert ID
                alert_data['alert'] = {'id': 'unknown_alert'}
                alert_data['threat'] = {'id': 'unknown_alert'}

        # Run enhanced multi-agent triage analysis
        print(f"\n{'='*60}")
        print("STARTING ENHANCED MULTI-AGENT TRIAGE ANALYSIS")
        print(f"{'='*60}")
        ...
        # Add input validation info to metadata
        ...
        results['api_metadata'] = {
            ...
            'mcp_integration': True,
            'threat_intelligence_sources': ['VirusTotal', 'Check Point', 'Cyberint'],
        ...
        print(f"Traceback: {traceback.format_exc()}")
        raise HTTPException(status_code=500, detail=f"Enhanced triage analysis failed: {str(e)}")
    ...
    async def run_all_agents_with_separate_data(self, source: str, edr_data: Dict[str, Any], gnn_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {
            "prediction": {
                ...
                "confidence": (1 - round(consolidated_score / 100.0, 4)) * 100,
...
async def run_supervisor_agent(
    ...
    """
    RAW JSON ONLY - Modified for specialized agent focus with raw confidence scores.
    Uses a single JSON payload for both EDR (alert_json) and GNN (gnn_json).
    Final decision is based 100% on the specialized agent determined by source mapping.
    Other agents run for informational purposes only.
    
    NEW: Uses raw confidence scores as probability of predicted class - no inversion or caps.
    ...
    try:
        ...
        # 3) Flexible processing (updated to match your new FlexibleAlertInput)
        edr_flexible = None
        gnn_flexible = None

        try:
            edr_flexible = FlexibleAlertInput(**edr_json)
            edr_processed = edr_flexible.to_raw_format()  # Updated method name
        except Exception as e:
        ...
        try:
            ...
            gnn_processed = gnn_flexible.to_raw_format()  # Updated method name
        except Exception as e:
...
if __name__ == "__main__":
    uvicorn.run("app-mcp-in:app", host="0.0.0.0", port=2030 , reload=True)