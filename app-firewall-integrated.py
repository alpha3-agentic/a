...
class ScoringTool:
    ...
    @classmethod
    def _get_virustotal_data(cls, sha256_hash: str, api_key: str = None) -> Dict[str, Any]:
        """
        Fetch VirusTotal data for given SHA256 hash
        """
        if not api_key:
        ...
        if not api_key:
            print("No VirusTotal API key available")
            return {}
            
        if not sha256_hash:
            print("No SHA256 hash provided")
            return {}
        ...
        try:
            ...
            print(f"Fetching VirusTotal data for hash: {sha256_hash}")
            
            # Add rate limiting - VirusTotal free tier allows 4 requests per minute
            ...
            if response.status_code == 200:
                ...
                print(f"Hash not found in VirusTotal: {sha256_hash}")
            ...
            print("VirusTotal API request timeout")
            return {"error": "timeout"}
        except requests.exceptions.RequestException as e:
            print(f"VirusTotal API request error: {str(e)}")
            return {"error": f"request_error: {str(e)}"}
        except Exception as e:
            print(f"Unexpected error fetching VirusTotal data: {str(e)}")
    ...
    def _extract_sha256_from_alert(cls, flat: Dict[str, Any]) -> str:
        ...
        """
        # Try multiple possible locations for SHA256 hash
        possible_keys = [
            "file.hashes.sha256",
            "threat.sha256", 
            "file.sha256",
    ...
    def score_agent2(cls, flat: Dict[str, Any]) -> Dict[str, dict]:
        ...
        sha256_hash = cls._extract_sha256_from_alert(flat)
        ...
        vt_data = cls._get_virustotal_data(sha256_hash)
        ...
        if not vt_data or "error" in vt_data:
            ...
            scores["vt_error"] = {
                "value": error_msg,
                "risk_score": 10,  # Small risk score for unknown files
                "description": f"VirusTotal lookup failed: {error_msg}"
            }
            return scores
        ...
        if not attributes:
            print("No attributes found in VirusTotal response")
            scores["vt_no_attributes"] = {
                "value": "No attributes",
                "risk_score": 5,
                "description": "VirusTotal response contained no file attributes"
            }
            return scores

        print(f"Processing VirusTotal attributes for analysis...")

        # 1. Last analysis stats - malicious detections
        ...
        if malicious_count > 0:
            ...
            print(f"Malicious detections: {malicious_count} -> score: {risk_score}")
        ...
        if suspicious_count > 0:
            ...
            print(f"Suspicious detections: {suspicious_count} -> score: {risk_score}")

        # 3. Harmless detections (reduces risk)
        harmless_count = last_analysis_stats.get("harmless", 0)
        if harmless_count > 0:
            risk_score = -min(2 * math.log2(harmless_count + 1), 10)
            scores["vt_harmless_detections"] = {
                "value": harmless_count,
                "risk_score": risk_score,
                "description": f"VirusTotal harmless detections: {harmless_count}"
            }
            print(f"Harmless detections: {harmless_count} -> score: {risk_score}")

        # 4. Timeout detections (potential evasion)
        timeout_count = last_analysis_stats.get("timeout", 0)
        if timeout_count > 0:
            risk_score = min(5 * math.log2(timeout_count + 1), 15)
            scores["vt_timeout_detections"] = {
                "value": timeout_count,
                "risk_score": risk_score,
                "description": f"VirusTotal timeout detections: {timeout_count}"
            }
            print(f"Timeout detections: {timeout_count} -> score: {risk_score}")

        # 5. File reputation score
        ...
        if reputation < 0:
            ...
            scores["vt_negative_reputation"] = {
                "value": reputation,
                "risk_score": risk_score,
                "description": f"VirusTotal negative reputation score: {reputation}"
            }
            print(f"Negative reputation: {reputation} -> score: {risk_score}")

        # 6. Popular threat classification
        popular_threat_classification = attributes.get("popular_threat_classification", {})
        if popular_threat_classification:
            threat_categories = popular_threat_classification.get("popular_threat_category", [])
            if threat_categories:
                # Check for high-risk categories
                high_risk_categories = ["trojan", "malware", "virus", "backdoor", "ransomware"]
                for category_info in threat_categories:
                    if isinstance(category_info, dict):
                        category = category_info.get("value", "").lower()
                        count = category_info.get("count", 0)
                        
                        if category in high_risk_categories:
                            risk_score = min(20 + (count * 2), 40)
                            scores["vt_threat_category"] = {
                                "value": f"{category} ({count})",
                                "risk_score": risk_score,
                                "description": f"VirusTotal threat category: {category} with {count} detections"
                            }
                            print(f"Threat category: {category} ({count}) -> score: {risk_score}")
                            break
        ...
        if threat_severity:
            ...
            if "HIGH" in severity_level:
                scores["vt_high_severity"] = {
                    ...
                    "risk_score": 35,
                    "description": f"VirusTotal high threat severity: {severity_level}"
                }
                print(f"High threat severity: {severity_level} -> score: 35")
                ...
                print(f"Medium threat severity: {severity_level} -> score: 20")

        # 8. Crowdsourced YARA results
        crowdsourced_yara = attributes.get("crowdsourced_yara_results", [])
        if crowdsourced_yara and len(crowdsourced_yara) > 0:
            yara_count = len(crowdsourced_yara)
            risk_score = min(yara_count * 3, 25)
            scores["vt_yara_matches"] = {
                "value": yara_count,
                "risk_score": risk_score,
                "description": f"VirusTotal YARA rule matches: {yara_count}"
            }
            print(f"YARA matches: {yara_count} -> score: {risk_score}")

        # 9. Signature verification
        signature_info = attributes.get("signature_info", {})
        if signature_info:
            signers_details = signature_info.get("signers details", [])
            if signers_details:
                # Check for expired or invalid signatures
                for signer in signers_details:
                    if isinstance(signer, dict):
                        status = signer.get("status", "").lower()
                        if "expired" in status or "invalid" in status or "not time valid" in status:
                            scores["vt_invalid_signature"] = {
                                "value": status,
                                "risk_score": 15,
                                "description": f"VirusTotal invalid/expired signature: {status}"
                            }
                            print(f"Invalid signature: {status} -> score: 15")
                            break

        # 10. Unique sources count (high distribution = suspicious)
        unique_sources = attributes.get("unique_sources", 0)
        if unique_sources > 100:
            risk_score = min((unique_sources - 100) / 10, 20)
            scores["vt_high_distribution"] = {
                "value": unique_sources,
                "risk_score": risk_score,
                "description": f"VirusTotal high distribution: {unique_sources} unique sources"
            }
            print(f"High distribution: {unique_sources} sources -> score: {risk_score}")
        ...
        scores["vt_api_summary"] = {
            "value": f"API lookup completed",
            "risk_score": 0,  # Don't double-count
            "description": f"VirusTotal API analysis completed with {len(scores)} indicators"
        }
    ...
    def score_agent1(cls, flat: Dict[str, Any]) -> Dict[str, dict]:
        """Agent1: Heuristic-based scoring with better type handling"""
        scores = {}
        
        print("=== AGENT1 SCORING START ===")
        print(f"Available keys: {list(flat.keys())}")

        # 1. Severity scoring - handle various types
        sev = 0
        ...
        # 2. File signing verification - handle bool/string
        ...
        vc_raw = flat.get("file.signature.certificate.status", False)
        vc = bool(vc_raw) if vc_raw not in [None, ""] else False
        ...
        # 5. Command line patterns - handle None/non-string values
        ...
        # 6. Threat confidence - handle int/string types
        ...
        # 7. Detection engine - handle nested structures safely
...
async def triage_alert(
    ...
    """
    Upload a JSON alert file or send JSON data directly for triage analysis with risk scoring.
    Supports flexible input formats with automatic field mapping.
    """
    global triage_agent
    ...
    try:
        ...
        try:
            ...
            # Fallback: use raw data with minimal validation
            alert_data = raw_data
            validation_success = False
            
            if not any(key in alert_data for key in ['alert', 'threat', 'id', 'alert_id']):
                # Add a synthetic alert ID
                alert_data['alert'] = {'id': 'unknown_alert'}
                alert_data['threat'] = {'id': 'unknown_alert'}

        # Run triage analysis
        results = triage_agent.analyze_alert(alert_data)
        
        # Add input validation info to metadata - now variables are always defined
        results['metadata']['input_processing'] = {
            'validation_success': validation_success,
            'flexible_parsing_used': validation_success and flexible_input is not None,
            ...
            'mapped_alert_id': flexible_input.alert_id if flexible_input else None
        }
...
async def triage_alert(
    source: str = Query(..., description="Source type (e.g., 'firewall', 'endpoint', 'network')"),
    file: UploadFile = File(None),
    json_data: dict = Body(None),
...
class FlexibleAlertInput(BaseModel):
    """Flexible input model for various alert formats"""
    ...
    # Allow additional fields from nested JSON structures
    ...
    def to_raw_format(self) -> Dict[str, Any]:
        ...
        # Extract from nested structures first
        if self.file:
            result["file_size"] = self.file.get("size", 0) or 0
            ...
            cert_status = self.file.get("signature", {}).get("certificate", {}).get("status")
            result["is_valid_certificate"] = cert_status == "valid"
            result["verification_type"] = str(self.file.get("verification", {}).get("type", "unknown")).lower()
            
        if self.threat:
            result["threat_confidence"] = self.threat.get("confidence", 50) or 50
            result["detection_type"] = str(self.threat.get("detection", {}).get("type", "unknown")).lower()
        ...
        if self.device:
            result["os_name"] = str(self.device.get("os", {}).get("name", "unknown")).lower()
            result["device_type"] = str(self.device.get("type", "unknown")).lower()
            result["is_active"] = self.device.get("is_active", False)
            
        if self.process:
            result["is_fileless"] = self.process.get("is_fileless", False)
        ...
        if self.file_extension:
            result["file_extension"] = self.file_extension.lower()
        if self.file_extension_type:
            result["file_extension_type"] = self.file_extension_type.lower()
        if self.verification_type:
            result["verification_type"] = self.verification_type.lower()
        if self.is_valid_certificate is not None:
            result["is_valid_certificate"] = self.is_valid_certificate
        if self.threat_confidence is not None:
            result["threat_confidence"] = self.threat_confidence
        if self.detection_type:
            result["detection_type"] = self.detection_type.lower()
        if self.threat_classification:
            result["threat_classification"] = self.threat_classification.lower()
        if self.os_name:
            result["os_name"] = self.os_name.lower()
        if self.device_type:
            result["device_type"] = self.device_type.lower()
        if self.is_active is not None:
            result["is_active"] = self.is_active
        if self.is_fileless is not None:
            result["is_fileless"] = self.is_fileless
        if self.confidence_level:
            result["confidence_level"] = self.confidence_level.lower()
        if self.severity:
            result["severity"] = self.severity.lower()
        if self.verdict:
            result["verdict"] = self.verdict.lower()
            
        return result


class SinglePKLPredictor:
    """Predictor that works with a single PKL file"""
    ...
    def predict(self, input_data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if hasattr(self.model, 'feature_importances_'):
                ...
                for i, feat in enumerate(self.expected_features):
...
class Neo4jGraphManager:
        ...
        with self.driver.session(database=self.database) as session:
            try:
                ...
                node_methods = [
                    ...
                    ('ThreatIntel_CheckPoint', self._create_node_10_threat_intel_checkpoint),
                    ('ThreatIntel_VirusTotal', self._create_node_11_threat_intel_virustotal),
                ...
                relationship_methods = [
                    ...
                    ('HASH_ENRICHED_BY_TI_CP', self._create_rel_8_hash_enriched_by_ti_cp),
                    ('HASH_ENRICHED_BY_TI_VT', self._create_rel_9_hash_enriched_by_ti_vt),
    ...
    def _create_node_10_threat_intel_checkpoint(self, session, data):
        """10. ThreatIntel (Check Point) â€" key: enrichments[0].data.resource (optional)"""
        enrichments = data.get('enrichments', [])
        
        if enrichments and len(enrichments) > 0:
            cp_data = enrichments[0].get('data', {})
            resource = cp_data.get('resource')
            ...
            if resource:
                ...
                print(f"No resource found in enrichment data, skipping CheckPoint ThreatIntel creation")
        else:
            print(f"No enrichments found, skipping CheckPoint ThreatIntel creation")
...
class AgenticGraphRAG:
    ...
    def _generate_parallel_investigation_tasks(self, alert_id: str) -> List[InvestigationTask]:
        ...
        base_questions = [
            ...
            "What detection and analysis information exists from security tools?",
    ...
    try:
        ...
        try:
            ...
            if (len(result.get('qa_history', [])) < 3 and 
                ...
                try:
                    # Process context data asynchronously
                    flexible_input = FlexibleAlertInput(**context_data)
                    
                    # Run context enhancement in parallel if investigation was insufficient
...
class SupervisorAgent:
    ...
    async def run_edr_agent(self, edr_data: Dict[str, Any]) -> Dict[str, Any]:
        """Run EDR agent using the classifier with better error handling"""
        try:
            # Use safe prediction
            result = classifier.safe_predict(edr_data)
            
            # Handle error case
            if result.get('prediction', {}).get('predicted_verdict') == 'Error':
                return self._error_response("EDR", f"EDR prediction failed: {result['metadata']['error']}")
    ...
    async def run_gnn_agent(self, gnn_data: Dict[str, Any]) -> Dict[str, Any]:
        """Run GNN agent via the DynamicGNNPredictor."""
        try:
            ...
            score_pct = float(gnn_result.get("confidence", 0.0))  # already 0–100 from predictor

            # Map to your risk-score policy (unchanged policy)
            if verdict == "True Positive":
                risk_score = max(80.0, score_pct)
            elif verdict == "Escalate":
                risk_score = max(40.0, min(score_pct, 79.0))
            else:  # False Positive
                risk_score = min(score_pct, 30.0)

            return {
                "agent": "GNN",
                "score": int(risk_score),
                "verdict": verdict,
                "confidence": round(score_pct, 2),
                "probabilities": gnn_result.get("probabilities", {}),
                "mode": gnn_result.get("mode", "selfie"),
                "message": f"GNN Analysis ({gnn_result.get('mode', 'selfie')}): {verdict} with {score_pct:.1f}% confidence",
                "details": {
...
async def run_supervisor_agent(
    ...
    """
    RAW JSON ONLY.
    Uses a single JSON payload for both EDR (alert_json) and GNN (gnn_json).
    Accepted shapes:
    ...
    try:
        ...
        # 3) Flexible processing (keep your existing logic/classes)
        edr_flexible = None
        gnn_flexible = None

        try:
            edr_flexible = FlexibleAlertInput(**edr_json)
            edr_processed = edr_flexible.to_legacy_format()
        except Exception as e:
        ...
        try:
            ...
            gnn_processed = gnn_flexible.to_legacy_format()
        except Exception as e:
...
class PureInfoSummaryAgent:
    ...
    def _extract_key_points(self, info: str) -> List[str]:
        ...
        try:
            ...
            return clean_points[:6]  # Max 6 points
        except Exception as e:
...
# ---- Dynamic GNN: initialize the predictor and mount routes ----
try:
    if neo4j_driver:
...
if __name__ == "__main__":
    uvicorn.run("app-firewall-integrated:app", host="0.0.0.0", port=4000, reload=True)